...
public class GamePlayThread extends Thread {
	...
	private static final long MAX_TIME_TO_ATTACK_PLAYER = 1000; //1 sec.
	
	private static final long MAX_TIME_TO_INIT_PLAYER = 1000; //1 sec.
	
	private static final long MAX_TIME_TO_OPPONENT_ATTACK_PLAYER = 100; //0.1 sec.
	...
	private void fireInit(final Player player){
		...
		initThread.start();
		boolean isExceededLimitOfMemory = false;
		try{
			while (initThread.isAlive()){
				initThread.join(10);
				if (calcPlayerMemoryUsage(player) > MAX_MEMORY_TO_PLAYER){
					logger.info(player.getClass() + " - Player exceeded limit of memory on execution of init method.");
					isExceededLimitOfMemory = true;
				}
			}
		}catch (InterruptedException e) {
		}
		
		boolean isExceededTimeLimit = false;
		if (System.currentTimeMillis() - t1 > MAX_TIME_TO_INIT_PLAYER){
			isExceededTimeLimit = true;
			logger.info(player.getClass() + " - Player exceeded time limit on init method. Ignored.");
		}
	}
	
	private Attack fireAttack(final Player player, final World world){
		...
		try{
			while (attackThread.isAlive()){
				...
				if (calcPlayerMemoryUsage(player) > MAX_MEMORY_TO_PLAYER){
					logger.info(player.getClass() + " - Player exceeded limit of memory. Attack failed.");
					isExceededLimitOfMemory = true;
				}
			}
		}catch (InterruptedException e) {
		...
		boolean isExceededTimeLimit = false;
		if (System.currentTimeMillis() - t1 > MAX_TIME_TO_ATTACK_PLAYER){
			isExceededTimeLimit = true;
			logger.info(player.getClass() + " - Player exceeded time limit on attack method. Ignored.");
		}
		
		if (isExceededLimitOfMemory) return null;
		
		return attackThread.getAttack();
	}

	private void fireOpponentAttack(final Player player, final Flag opponentFlag, final Attack attack, final World world){
		...
		try{
			while (opponentAttackThread.isAlive()){
				...
				if (calcPlayerMemoryUsage(player) > MAX_MEMORY_TO_PLAYER){
					logger.info(player.getClass() + " - Player exceeded limit of memory on execution of opponentAttack method.");
					isExceededLimitOfMemory = true;
				}
			}
		}catch (InterruptedException e) {
		...
		boolean isExceededTimeLimit = false;
		if (System.currentTimeMillis() - t1 > MAX_TIME_TO_OPPONENT_ATTACK_PLAYER){
			isExceededTimeLimit = true;
			logger.info(player.getClass() + " - Player exceeded time limit on opponentAttack method. Ignored.");
		}
	}
	...
	public void run() {
		...
		FullWorldGenerator fullWorldGenerator = configuration
				.geFullWorldGenerator();
			...
			for (int i = 0; i < playerCount; i++) {
				...
				while (canAttack && t) {
					...
					try {
						...
						Attack attack = fireAttack(players[i], immutableWorld);
							...
							if (!successAttacked) {
								...
								for (int j = 0; j < playerCount; j++) {
									...
									if (!f.equals(playerFlag)
										...
										fireOpponentAttack(players[i], f, attack, immutableWorld);
									}
								}
							}
						}
